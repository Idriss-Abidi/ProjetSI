...
import { HiAdjustments, HiArrowRight, HiClipboardList, HiUserCircle } from "react-icons/hi";
      ...
      try {
        ...
        const filteredCandidatures = candidaturesData.filter(
          (candidature: { id_etudiant: number; statut: string }) =>
            candidature.id_etudiant === idEtudiant && candidature.statut === "En att"
        );
        ...
        const filteredEntretiens = entretiensData.filter(
          (entretien: { id_etudiant: number }) => entretien.id_etudiant === idEtudiant
        );
        setDataEntretiens(filteredEntretiens);

        const stagesResponse = await fetch("/db/stage_offre.json");
        const stagesData = await stagesResponse.json();
        const filteredStages = stagesData.filter((stage: { id_etudiant: number }) => stage.id_etudiant === idEtudiant);
      ...
      <div className="container m-auto p-8 ">
        <div className="hero bg-base-200 min-h-[400px]">
          <div className="hero-content flex-col md:flex-row-reverse">
            <img src="/login1.jpg" className="md:w-3/12 w-6/12 rounded-lg shadow-2xl" />
            <div className="w-9/12 ml-[7%] min-h-[250px]">
              <h1 className="text-5xl font-bold">Bienvenue !</h1>
              <div className="py-6 w-[80%]">
                <p>
                  Trouvez facilement des offres de stage et suivez l'évolution de vos candidatures tout au long du
                  processus.
                </p>
                ...
                <ul>
                  <li>
                    <strong>Explorez les offres de stage</strong> : Parcourez les offres publiées par les entreprises et
                    trouvez celles qui correspondent à vos compétences et vos intérêts.
                  </li>
                  <li>
                    <strong>Postulez à des offres</strong> : Envoyez votre candidature et montrez aux entreprises
                    pourquoi vous êtes le candidat idéal.
                  </li>
                  <li>
                    <strong>Suivez vos candidatures</strong> : Restez informé de l'état de vos candidatures et de
                    l'avancée du processus de sélection.
                  </li>
                </ul>
              </div>
            </div>
